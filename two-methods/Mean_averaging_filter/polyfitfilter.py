...
class Polyfitfilter:
    ...
    def FIR_FilterCoefficients(self,left_fit,right_fit):
        if ((self.first_run==True)|(self.confidence==0)):
...
#        print(weighting)
    ...
    def lineParametersQuestionable(self,left_fit,right_fit):
        ...
        if ((abs(left_fit[0]-self.left_filtered_coeff[0])>self.max_left_delta_coeff[0])&(not self.first_run)):
            left_fit=self.left_ext_filt_coeff
            self.acceptable_coeff=False
        if ((abs(left_fit[1]-self.left_filtered_coeff[1])>self.max_left_delta_coeff[1])&(not self.first_run)):
            left_fit=self.left_ext_filt_coeff
            self.acceptable_coeff=False
        if ((abs(left_fit[2]-self.left_filtered_coeff[2])>self.max_left_delta_coeff[2])&(not self.first_run)):
            left_fit=self.left_ext_filt_coeff
            self.acceptable_coeff=False
        if ((abs(right_fit[0]-self.right_filtered_coeff[0])>self.max_right_delta_coeff[0])&(not self.first_run)):
            right_fit=self.right_ext_filt_coeff
            self.acceptable_coeff=False
        if ((abs(right_fit[1]-self.right_filtered_coeff[1])>self.max_right_delta_coeff[1])&(not self.first_run)):
            right_fit=self.right_ext_filt_coeff
            self.acceptable_coeff=False
        if ((abs(right_fit[2]-self.right_filtered_coeff[2])>self.max_right_delta_coeff[2])&(not self.first_run)):
            right_fit=self.right_ext_filt_coeff
            self.acceptable_coeff=False
